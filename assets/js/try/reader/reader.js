...
(() => {
  function start() {
    ...
    const updatePill = () => { if (pill) pill.textContent = `${files.length} ${files.length===1 ? 'file' : 'files'}`; };
    ...
    function addFiles(list){
      ...
      if (added === 0 && list?.length) console.warn(`Only PDF files are allowed (max ${MAX_FILES}).`);
      results = []; if (statsBox) statsBox.innerHTML = '';
      updatePill(); renderList();
    }
    ...
    const rowHtml = (r,i) => `
      <div class="row">
        ...
        <div style="min-width:0">
          ...
          <div class="subtle mini" style="margin-top:.15rem">→ <code class="kbd">${r.newName}</code> in <code class="kbd">${r.folder}</code></div>
        </div>
    ...
    function renderResultsFolders(){
      if (!log) return;
      if(results.length===0){ log.innerHTML = '<p class="subtle mini">Run the demo to see results.</p>'; return; }
    ...
    function render(){ (viewMode==='folders') ? renderResultsFolders() : renderResultsList(); }
    ...
    function exportCSV(){
      ...
      const data = (results.length ? results.map(r => [r.name, r.newName, r.label, 'true'])
                                   : files.map(f => [f.name,'','', 'false']));
      if(!data.length) return;
      const csv = header.join(',')+'\n'+data.map(row=>row.map(v=> `"${clip(v)}"`).join(',')).join('\n');
      const blob = new Blob([csv], {type:'text/csv;charset=utf-8;'});
      const url = URL.createObjectURL(blob);
      const a=document.createElement('a'); a.href=url; a.download='summary.csv';
    ...
    btnClear?.addEventListener('click', ()=>{ files=[]; results=[]; if (input) input.value=''; updatePill(); renderList(); if (statsBox) statsBox.innerHTML=''; });
    btnSamples?.addEventListener('click', loadSamples);
    btnExport?.addEventListener('click', exportCSV);
    btnList?.addEventListener('click', ()=>{ viewMode='list'; btnList.setAttribute('aria-pressed','true'); btnFolders?.setAttribute('aria-pressed','false'); render(); });
    btnFolders?.addEventListener('click', ()=>{ viewMode='folders'; btnFolders.setAttribute('aria-pressed','true'); btnList?.setAttribute('aria-pressed','false'); render(); });
    copySetup?.addEventListener('click', async ()=> {
      if (!setupBlock) return;
      const text = setupBlock.innerText.replace(/\u00A0/g,' ');
      try { await navigator.clipboard.writeText(text); copySetup.textContent='Copied ✓'; setTimeout(()=> copySetup.textContent='Copy', 1200); }
      catch { copySetup.textContent='Copy failed'; setTimeout(()=> copySetup.textContent='Copy', 1200); }
    });
