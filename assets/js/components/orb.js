...
export class Orb {
  constructor(el, options = {}) {
    ...
    const defaults = {
      ...
      // NEW: “aliveness” toggles
      hueShift: true,              // ciclo HSL sutil en arco/glow
      hueSpeed: 0.04,              // velocidad del hue shift
      particleLayers: 3,           // profundidad de partículas (1–4 recomendado)
      trailStrength: 0.06,         // 0 desactiva estela; 0.04–0.08 sutil
      lightSweepEvery: 7.5,        // segundos entre barridos de luz (0 = off)
      cometChance: 0.003,          // probabilidad por frame (baja!)
      cometMax: 1,                 // máx cometas simultáneos
      chromaArc: 0.12,             // aberración cromática del arco (px)
      reactiveScroll: true,        // pulso reactividad a scroll
      reactiveCursor: true,        // pulso reactividad a velocidad del cursor
      lensDirt: 0.18,              // mezcla “suciedad de lente” (0–0.3)
    };
    ...
    // pre-render noise (una vez)
    this._noise = this._makeNoise(256); // NEW: un poco más grande
  ...
  _hsla(h,s,l,a=1){ return `hsla(${(h%360+360)%360},${s}%,${l}%,${a})`; } // NEW
  ...
  _sizeCanvas() {
    ...
    if (this.canvas.width !== w || this.canvas.height !== h) {
      ...
      // NEW: re-crear trail FBO
      this._trail = document.createElement('canvas');
      this._trail.width = w; this._trail.height = h;
  ...
  _setup() {
    ...
    if (this.cta) this.cta.addEventListener('mouseenter', () => this.pulseBoost = Math.max(this.pulseBoost, 1));
  ...
  _drawComets(ctx, baseR, brandHue){
    ...
    for (const k of this._comets){
      ...
      const col = this._hsla(brandHue, 70, 60, 0.5*a);
  ...
  _draw(now, dt){
    ...
    // NEW: estela (desenfoque multiplicativo muy leve)
    ...
    // Marca base → tomamos hue dinámico si hueShift
    ...
    // NEW: hue dinámico en torno a 265º (morado) → 185º (teal)
    const hue = this.OPT.hueShift ? (220 + Math.sin(t * this.OPT.hueSpeed * 2 * Math.PI) * 35) : 220;

    // ===== Fondo: doble halo con hue dinámico =====
    ...
    const bg2 = ctx.createRadialGradient(
      cw*.5 - this.parx*0.45, ch*.5 - this.pary*0.45, 18,
      cw*.5,                   ch*.5,                  Math.max(cw,ch)*0.88
    );
    bg2.addColorStop(0, this._hsla(hue-45, 70, 45, 0.11 + 0.05*Math.max(0,breath)));
    bg2.addColorStop(1, 'rgba(0,0,0,0)');
    ctx.fillStyle = bg2;
    ctx.beginPath(); ctx.arc(cw*.5, ch*.5, minSide*.64, 0, Math.PI*2); ctx.fill();

    // ===== Disco principal =====
    const disc = ctx.createRadialGradient(
      cw*.5 + this.parx*0.55, ch*.5 + this.pary*0.55, 8,
      cw*.5,                  ch*.5,                   baseR*0.96
    );
    ...
    // ===== Arco de marca con aberración cromática ligera =====
    const ang = this.rm ? 0 : t*0.7;
    ctx.save();
    ctx.translate(cw*.5, ch*.5);
    ctx.rotate(ang + breath*0.5);

    // gradiente principal (HSL si hueShift)
    ...
    // “canal R” desplazado para aberración sutil
    ...
    if (!this.rm){
      ...
      for (let L=0; L<layers; L++){
        ...
        for (let i=0;i<N;i++){
          ...
          const x = cw*.5 + Math.cos(a)*rr + this.parx*0.12*z + jx*z;
          const y = ch*.5 + Math.sin(a)*rr + this.pary*0.12*z + jy*z;
          const size = 1.6*z;
    ...
    if (!this.rm && Math.random() < this.OPT.cometChance){
      this._spawnComet(cw*.5, ch*.5, baseR*0.7, ang + Math.random()*Math.PI*2, t);
    }
    this._updateComets(dt);
    this._drawComets(ctx, baseR, hue);

    // ===== Textura sutil + “lens dirt” =====
    if (this.OPT.noiseStrength > 0){
      const s = this._noise.width;
      const nx = (cw*.5 - baseR) + (Math.sin(t*0.07)*8 + this.parx*0.2);
      const ny = (ch*.5 - baseR) + (Math.cos(t*0.06)*8 + this.pary*0.2);
